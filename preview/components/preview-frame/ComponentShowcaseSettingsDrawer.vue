  ...
  <div class="showcase__settings-drawer">
    <div class="showcase__settings-drawer-toolbar">
      <button
        ...
        <template v-if="!isSettingsDrawerExpanded">Show code and details</template>
        <template v-else>Hide code and details</template>
      </button>
      ...
      <span class="showcase__render-state">{{ selectedStateTitle }} on {{ getSelectedCssBreakpointTitle().toLowerCase() }}</span>
    </div>
...
export default {
  ...
  computed: {
    artifactoryHref() {
      // TODO: Implement
      return "Not implemented in DvM";//`${CONFIG.ARTIFACTORY_URL}/${this.showcaseRepo.Name}`;
    },

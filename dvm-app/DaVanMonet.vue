  ...
  <div class="davanmonet-app">
    ...
    <header class="davanmonet-header">
      <a :href="publicPath" class="davanmonet-header-logolink">
        ...
        <span
          class="davanmonet-header-logolinktext"
          v-if="projectConfig && projectConfig.project_info"
          v-html="projectConfig.project_info.name"
          >DaVanMonet</span
        >
      </a>
      ...
      <a
        v-if="
          projectConfig &&
          projectConfig.project_info &&
          projectConfig.project_info.repourl
        "
        :href="projectConfig.project_info.repourl"
        class="davanmonet-header-repository-link"
    ...
    <div class="davanmonet-pagecontainer">
      ...
      ></main-content>
      <div v-if="configLoaded == false">Loading...</div>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      maincontent: {
        ...
        Preamble: null,
        MainBody:
          "<p>This project does not yet have a startpage. Create a index.md file in the root folder of your style source</p>",
      },
      versionData: {},
      projectConfig: {},
      contentIndex: {},
  ...
  computed: {
    ...
    logoPath: function () {
      ...
      else return (this.publicPath + "/static/logo.svg").replace("//", "/");
    },
  },
  ...
  methods: {
      ...
      if (_vue.projectConfig.project_info.theme_style !== "default") {
        $("head").append(
          '<link rel="stylesheet" type="text/css" href="' +
            _vue.projectConfig.project_info.theme_style +
            '" />'
        );
      }
      if (_vue.projectConfig.project_info.name) {
        $(".davanmonet-header-logolinktext").text(
          _vue.projectConfig.project_info.name
        );
      }
      if (_vue.projectConfig.project_info.logo) {
        $(".davanmonet-header-logo").attr(
          "src",
          _vue.projectConfig.project_info.logo
        );
      }
    },
    ...
    async loadPage(_vue, path) {
      if (path.length > 0) {
        ...
        if (
          pagedata != null &&
          ((pagedata.id && pagedata.id.length > 0) ||
            (pagedata.title && pagedata.title.length > 0))
        ) {
    ...
    parseLocationAndNavigate() {
      ...
      if (
        (this.isLocalhost || this.projectConfig.directories.use_hash) &&
        hash.length > 0
      ) {
